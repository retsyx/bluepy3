...
static void resp_begin(const char *rsptype) { printf("%s=$%s", tag_RESPONSE, rsptype); }

static void send_sym(const char *tag, const char *val) { printf(RESP_DELIM "%s=$%s", tag, val); }

static void send_uint(const char *tag, unsigned int val) {
  printf(RESP_DELIM "%s=h%X", tag, val);
}

static void send_str(const char *tag, const char *val) { printf(RESP_DELIM "%s='%s", tag, val); }

static void send_data(const unsigned char *val, size_t len) {
  printf(RESP_DELIM "%s=b", tag_DATA);
  while (len-- > 0) printf("%02X", *val++);
}
...
static void send_addr(const struct mgmt_addr_info *addr) {
  ...
  while (len-- > 0) printf("%02X", val[len]);
...
static void events_handler(const uint8_t *pdu, uint16_t len, gpointer user_data) {
  ...
  if (evt == ATT_OP_HANDLE_NOTIFY) return;
...
static void connect_cb(GIOChannel *io, GError *err, gpointer user_data) {
  ...
  g_attrib_register(attrib, ATT_OP_HANDLE_NOTIFY, GATTRIB_ALL_HANDLES, events_handler, attrib,
                    NULL);
  g_attrib_register(attrib, ATT_OP_HANDLE_IND, GATTRIB_ALL_HANDLES, events_handler, attrib, NULL);
  g_attrib_register(attrib, ATT_OP_FIND_INFO_REQ, GATTRIB_ALL_HANDLES, gatts_find_info_req,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_FIND_BY_TYPE_REQ, GATTRIB_ALL_HANDLES, gatts_find_by_type_req,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_READ_BY_TYPE_REQ, GATTRIB_ALL_HANDLES, gatts_read_by_type_req,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_READ_REQ, GATTRIB_ALL_HANDLES, gatts_read_req, attrib, NULL);
  g_attrib_register(attrib, ATT_OP_READ_BLOB_REQ, GATTRIB_ALL_HANDLES, gatts_read_blob_req,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_READ_MULTI_REQ, GATTRIB_ALL_HANDLES, gatts_read_multi_req,
                    attrib, NULL);
  ...
  g_attrib_register(attrib, ATT_OP_SIGNED_WRITE_CMD, GATTRIB_ALL_HANDLES, gatts_signed_write_cmd,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_PREP_WRITE_REQ, GATTRIB_ALL_HANDLES, gatts_prep_write_req,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_EXEC_WRITE_REQ, GATTRIB_ALL_HANDLES, gatts_exec_write_req,
                    attrib, NULL);
  g_attrib_register(attrib, ATT_OP_MTU_REQ, GATTRIB_ALL_HANDLES, gatts_mtu_req, attrib, NULL);
...
static void char_read_by_uuid_cb(guint8 status, const guint8 *pdu, guint16 plen,
                                 gpointer user_data) {
  struct characteristic_data *char_data = user_data;
  struct att_data_list *list;
  int i;

  if (status == ATT_ECODE_ATTR_NOT_FOUND && char_data->start != char_data->orig_start) {
...
static void cmd_exit(int argcp, char **argvp) { g_main_loop_quit(event_loop); }
...
static gboolean channel_watcher(GIOChannel *chan, GIOCondition cond, gpointer user_data) {
  ...
  if (chan == iochannel) disconnect_io();
...
static void cmd_connect(int argcp, char **argvp) {
  ...
  iochannel = gatt_connect(opt_src, opt_dst, opt_dst_type, opt_sec_level, opt_psm, opt_mtu,
                           connect_cb, &gerr);
...
static void cmd_read_uuid(int argcp, char **argvp) {
  ...
  if (argcp < 2 || bt_string_to_uuid(&uuid, argvp[1]) < 0) {
    resp_error(err_BAD_PARAM);
    return;
  }
  ...
  gatt_read_char_by_uuid(attrib, start, end, &char_data->uuid, char_read_by_uuid_cb, char_data);
}
...
static void cmd_char_write(int argcp, char **argvp) { cmd_char_write_common(argcp, argvp, 0); }
...
static void cmd_sec_level(int argcp, char **argvp) {
  ...
  if (conn_state != STATE_CONNECTED) return;
...
static void set_mode_complete(uint8_t status, uint16_t length, const void *param,
                              void *user_data) {
  if (status != MGMT_STATUS_SUCCESS) {
    DBG("status returned error : %s (0x%02x)", mgmt_errstr(status), status);
...
static bool add_remote_oob_data(uint16_t index, const bdaddr_t *bdaddr, const uint8_t addr_type,
                                const char *hash192, const char *rand192, const char *hash256,
                                const char *rand256) {
...
static bool read_local_oob_data(uint16_t index) {
  ...
  if (mgmt_send(mgmt_master, MGMT_OP_READ_LOCAL_OOB_EXT_DATA, mgmt_ind, sizeof(cp), &cp,
                read_local_oob_data_complete, NULL, NULL) == 0) {
    resp_error(err_SEND_FAIL);
    return false;
  }
  return true;
...
static void cmd_pair(int argcp, char **argvp) {
  ...
  if (mgmt_send(mgmt_master, MGMT_OP_PAIR_DEVICE, mgmt_ind, sizeof(cp), &cp, pair_device_complete,
                NULL, NULL) == 0) {
...
static void unpair_device_complete(uint8_t status, uint16_t length, const void *param,
                                   void *user_data) {
  if (status != MGMT_STATUS_SUCCESS) {
    DBG("status returned error : %s (0x%02x)", mgmt_errstr(status), status);
    resp_mgmt_err(status);
...
static void cmd_unpair(int argcp, char **argvp) {
  ...
  if (mgmt_send(mgmt_master, MGMT_OP_UNPAIR_DEVICE, mgmt_ind, sizeof(cp), &cp,
...
static void scan(bool start) {
  ...
  if (mgmt_send(mgmt_master, opcode, mgmt_ind, sizeof(cp), &cp, scan_cb, NULL, NULL) == 0) {
...
static gboolean hci_monitor_cb(GIOChannel *chan, GIOCondition cond, gpointer user_data) {
  ...
  switch (type) {
    case HCI_COMMAND_PKT: {
      hci_command_hdr *ch;
      if ((r = g_io_channel_read_chars(chan, (gchar *)buf, HCI_COMMAND_HDR_SIZE, &len, &err)) !=
          G_IO_STATUS_NORMAL) {
      ...
      if ((r = g_io_channel_read_chars(chan, (gchar *)ptr, ch->plen, &len, &err)) !=
          G_IO_STATUS_NORMAL) {
        if (err) DBG("g_io_channel_read_chars() reports state %d: %s", r, err->message);
        return TRUE;
      }
    ...
    case HCI_EVENT_PKT: {
      hci_event_hdr *eh;
      if ((r = g_io_channel_read_chars(chan, (gchar *)buf, HCI_EVENT_HDR_SIZE, &len, &err)) !=
          G_IO_STATUS_NORMAL) {
        if (err) DBG("g_io_channel_read_chars() reports state %d: %s", r, err->message);
        return TRUE;
      }
      ...
      if ((r = g_io_channel_read_chars(chan, (gchar *)ptr, eh->plen, &len, &err)) !=
          G_IO_STATUS_NORMAL) {
        if (err) DBG("g_io_channel_read_chars() reports state %d: %s", r, err->message);
        return TRUE;
      }
      switch (eh->evt) {
        case EVT_CMD_COMPLETE: {
          ...
          // DBG("command complete (0x%02x|0x%04x) 0x%02x 0x%02x", cmpl->ncmd, cmpl->opcode,
          // *(uint8_t *)(ptr+3), *(uint8_t *)(ptr+4));
        } break;
        ...
        case EVT_LE_META_EVENT: {
          ...
          switch (meta->subevent) {
            case EVT_LE_ADVERTISING_REPORT: {
              ...
              if (0) {
                int i = 0;
                for (i = 0; i < ev->length; i++) DBG("buf: %02x", ev->data[i]);
              }
              ...
              if (conn_state == STATE_SCANNING) {
                ...
                if (ev->length) send_data(ev->data, ev->length);
                resp_end();
              }
            } break;
...
static struct {
  ...
  const char *desc;
    ...
    {"conn", cmd_connect, "[address [address type [interface]]]", "Connect to a remote device"},
    {"disc", cmd_disconnect, "", "Disconnect from a remote device"},
    {"svcs", cmd_primary, "[UUID]", "Primary Service Discovery"},
    {"incl", cmd_included, "[start hnd [end hnd]]", "Find Included Services"},
    ...
    {"le", cmd_le, "[on | off]", "Control LE feature on the controller"},
    {"remote_oob", cmd_add_oob, "address [[C_192 c192] [R_192 r192]] [[C_256 c256] [R_256 r256]]",
     "Add OOB data for remote address"},
    {"local_oob", cmd_read_oob, "", "Read local OOB data"},
    {"pairable", cmd_pairable, "[on | off]", "Control PAIRABLE feature on the controller"},
    {"pair", cmd_pair, "", "Start pairing with the device"},
    {"unpair", cmd_unpair, "", "Start unpairing with the device"},
    {"scan", cmd_scan, "", "Start scan"},
    {"scanend", cmd_scanend, "", "Force scan end"},
    {"pasv", cmd_pasv, "", "Start passive scan"},
    {"pasvend", cmd_pasvend, "", "Force passive scan end"},
    {NULL, NULL, NULL}};
...
static void parse_line(char *line_read) {
  ...
  for (i = 0; commands[i].cmd; i++)
    if (strcasecmp(commands[i].cmd, argvp[0]) == 0) break;
...
static void read_version_complete(uint8_t status, uint16_t length, const void *param,
                                  void *user_data) {
...
static void mgmt_device_connected(uint16_t index, uint16_t length, const void *param,
                                  void *user_data) {
...
static void mgmt_device_found(uint16_t index, uint16_t length, const void *param,
                              void *user_data) {
  const struct mgmt_ev_device_found *ev = param;
  // const uint8_t *val = ev->addr.bdaddr.b;
  assert(length == sizeof(*ev) + ev->eir_len);
  // DBG("Device found: %02X:%02X:%02X:%02X:%02X:%02X type=%X flags=%X", val[5], val[4], val[3],
  // val[2], val[1], val[0], ev->addr.type, ev->flags);

  // Result sometimes sent too early
  if (conn_state != STATE_SCANNING) return;
